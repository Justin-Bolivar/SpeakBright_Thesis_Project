...
class CardRankingList extends ConsumerStatefulWidget {
  final String
      selectedCategory; // Receive selectedCategory from CardRankingMenu

  const CardRankingList({super.key, required this.selectedCategory});
...
class _CardRankingListState extends ConsumerState<CardRankingList> {
  ...
  void initState() {
    ...
    studentID = ref
        ...
        .state; // Get the student ID from provider
    _loadCategoryData(
        widget.selectedCategory); // Load category data for selected category
  }
                        ...
                        child: categoryData.isEmpty
                            ...
                            : Padding(
                                ...
                                child: ReorderableListView(
                                  shrinkWrap: true,
                                  children:
                                      categoryData.map((DocumentSnapshot card) {
                                    final colorIndex =
                                        categoryData.indexOf(card) %
                                            boxColors.length;
                                    return Container(
                                      key: Key(card.id),
                                      height: 60,
                                  ...
                                  onReorder: (int oldIndex, int newIndex) {
                                    setState(() {
                                      ...
                                      final DocumentSnapshot item =
                                          categoryData.removeAt(oldIndex);
                                      categoryData.insert(newIndex, item);
                                    });
                                    _updateRank(
                                        categoryData[newIndex].id,
                                        newIndex +
                                            1); // Update the rank of the moved card
                                  },
                                ),
                              ),
                      ),
